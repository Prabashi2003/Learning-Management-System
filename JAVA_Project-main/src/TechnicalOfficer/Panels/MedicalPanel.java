...
public class MedicalPanel extends JPanel {
    ...
    public void initUi() {
        ...
        // TEXTFIELD - MEDICAL ID
        attendanceId = new JTextField();
        attendanceId.setBounds(450, 240, 90, 20);
        add(attendanceId);

        // BUTTON - DELETE
        deleteBtn = new JButton("DELETE");
        deleteBtn.setBounds(550, 240, 90, 20);
        deleteBtn.addActionListener(e -> deleteMedicalRecord());
        add(deleteBtn);
    }
    ...
    public void deleteMedicalRecord() {
        String medicalIdText = attendanceId.getText();

        if (medicalIdText == null || medicalIdText.trim().isEmpty()) {
            JOptionPane.showMessageDialog(this, "Please enter a valid Medical ID.");
            return;
        }
        ...
        try {
            ...
            // 1. Get atten_id from medical table
            String selectQuery = "SELECT atten_id FROM medical WHERE medical_id=?";
            PreparedStatement selectStmt = con.prepareStatement(selectQuery);
            selectStmt.setInt(1, medicalId);
            ResultSet rs = selectStmt.executeQuery();

            if (!rs.next()) {
                JOptionPane.showMessageDialog(this, "No medical record found for the given ID.");
                rs.close();
                selectStmt.close();
                con.close();
                return;
            ...
            int attenId = rs.getInt("atten_id");
            rs.close();
            selectStmt.close();

            // 2. Delete from medical table
            String deleteQuery = "DELETE FROM medical WHERE medical_id=?";
            PreparedStatement deleteStmt = con.prepareStatement(deleteQuery);
            deleteStmt.setInt(1, medicalId);
            ...
            String updateQuery = "UPDATE attendance SET atten_status='absent' WHERE atten_id=?";
            PreparedStatement updateStmt = con.prepareStatement(updateQuery);
            updateStmt.setInt(1, attenId);
            updateStmt.executeUpdate();
            updateStmt.close();

            JOptionPane.showMessageDialog(this, "Medical record deleted and attendance status set to absent.");

            con.close();
        } catch (NumberFormatException e) {
            JOptionPane.showMessageDialog(this, "Invalid Medical ID format.");
        } catch (SQLException e) {
            e.printStackTrace();
            JOptionPane.showMessageDialog(this, "Database error: " + e.getMessage());
        }
