...
public class UploadMarksExams extends javax.swing.JFrame {
    ...
    public UploadMarksExams(String userId) {
        ...
        tableViewMarks();
        SwingUtilities.invokeLater(() -> {//threading machanism in swing application
            setComboBox(userId);//if we need to change ui we need to do that through edt(event dispatch thread)
        });//invoke later used to make this fast in edt
    ...
    private void Marks_tableMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_Marks_tableMouseClicked
        ...
        if (rowIndex >= 0) {
            ...
            assesmnent_2.setText(model.getValueAt(rowIndex, 9).toString());
            Mid_term.setText(model.getValueAt(rowIndex, 10).toString());
            final_T.setText(model.getValueAt(rowIndex, 11).toString());
            final_P.setText(model.getValueAt(rowIndex, 12).toString());
        }
